...
from utils.align_utils import get_reference_facial_points, warp_and_crop_face
...
def process(img, facial_5_points, output_size):
    ...
    facial_points = []
    for i in range(len(facial_5_points)//2):
        facial_points.append([facial_5_points[i*2], facial_5_points[i*2 + 1]])
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        if args.save_image:
            for b in dets:
                ...
                # landms
                cv2.circle(img_raw, (b[5], b[6]), 2, (0, 0, 255), 4)
                cv2.circle(img_raw, (b[7], b[8]), 2, (0, 255, 255), 4)
                cv2.circle(img_raw, (b[9], b[10]), 2, (255, 0, 255), 4)
                cv2.circle(img_raw, (b[11], b[12]), 2, (0, 255, 0), 4)
                cv2.circle(img_raw, (b[13], b[14]), 2, (255, 0, 0), 4)
            # save image
        ...
        # k = 0
        # for facial_5_points in facial_5_points_many:
        #     dst_img = process(img_raw, facial_5_points, (112,112))
        #     if (not os.path.isdir(dirname)):
        #         os.mkdir(dirname)
        #     cv2.imwrite(os.path.join(dirname, "{}_".format(k) + os.path.basename(img_name)), dst_img)
        #     k += 1